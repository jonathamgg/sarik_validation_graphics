	   ...
	   START=0
           ...
           # Verifica se o pod usa imagem Alpine
            if [[ $(kubectl exec -it $pod -n $namespace -- cat /etc/os-release | grep -c "Alpine") -ne 0 ]]; then
                ...
                latencia=$(kubectl exec -it $pod -n $namespace -- sh -c "time curl https://www.google.com > teste.txt" | grep -E 'real' | awk '{print $2 "" $3}' | sed 's/[^0-9.]//g')
            else                
                # Usando o time para definir um limite de tempo em todos os comandos.

                latencia=$(kubectl exec -it $pod -n $namespace -- bash -c "time curl https://www.google.com > teste.txt" | grep -E 'real' | awk '{print $2 " " $3}' | sed 's/[^0-9.]//g')
            fi
