...
echo "Data; Group; Nome_POD; WGET" > metricas_tempo_resposta.csv
echo "Data,Group,Nome_POD,WGET" > metricas_para_calculo.csv
...
for pod in ${pods[@]}; do
    ...
    echo ""
        ...
        echo "Data; Group; Nome_POD; WGET" > metricas_tempo_resposta_pod_$pod.csv

	echo "Group,WGET" > metricas_para_R_pod_$pod.csv
	   ...
	   # obtendo horario inicio
		...
		latencia=$(kubectl exec -it $pod -n $namespace -- sh -c "time wget https://www.google.com" | grep -E 'real' | awk '{print $2 "" $3}' | sed 's/[^0-9.]//g')
            else
                # Usando o time para definir um limite de tempo em todos os comandos.

                latencia=$(kubectl exec -it $pod -n $namespace -- bash -c "time wget https://www.google.com" | grep -E 'real' | awk '{print $2 " " $3}' | sed 's/[^0-9.]//g')
            fi
           ...
           RESULT_TR_WGET=$(echo "$latencia")

           ####################################  WGET ################################
           ####################  SAIDA DOS RESULTADOS INDIVIDUAL ######################

           echo "$(date); $Group; $pod; $RESULT_TR_WGET" >> metricas_tempo_resposta_pod_$pod.csv

	   echo "$Group,$RESULT_TR_WGET" >> metricas_para_R_pod_$pod.csv
           ...
           echo "$(date); $Group; $pod; $RESULT_TR_WGET" >> metricas_tempo_resposta.csv
	   
	   echo "$(date +%d-%m-%Y),$Group,$pod,$RESULT_TR_WGET" >> metricas_para_calculo.csv
