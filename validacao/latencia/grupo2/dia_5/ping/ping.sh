	   ...
	   START=0
           ...
           # Verifica se o pod usa imagem Alpine
            if [[ $(kubectl exec -it $pod -n $namespace -- cat /etc/os-release | grep -c "Alpine") -ne 0 ]]; then
                ...
                latencia=$(kubectl exec -it $pod -n $namespace -- time ping -c 8 $CLUSTER_IP | grep -E 'real' | awk '{print $3""$4}' | sed 's/[^0-9.]//g')
                echo $latencia

		#latencia=$(echo "$latencia" | sed 's/\./,/g')
            else
                # Usando o time para definir um limite de tempo em todos os comandos.
                latencia=$(kubectl exec -it $pod -n $namespace -- time ping -c 8 $CLUSTER_IP | grep -E 'sys' | awk '{print $3}' | sed 's/[^0-9.]//g')
		echo $latencia
		#latencia=$(echo "$latencia" | sed 's/\./,/g')
            fi
