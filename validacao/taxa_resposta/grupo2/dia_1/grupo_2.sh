...
echo "Data; Group; Nome_POD; PING; WGET; CURL; APT" > metricas_tempo_resposta.csv
echo "Data,Group,Nome_POD,PING,WGET,CURL,APT" > metricas_para_calculo.csv
...
for pod in ${pods[@]}; do
	...
	echo "Group,PING,WGET,CURL,APT" > metricas_para_R_pod_$pod.csv
	   ...
	   DIFFERENCE=$(echo "scale=2; $DIFFERENCE" | bc | awk '{printf "%.1f", $0}' | sed 's/.0/./g')
	   ...
	   START=""
           ...
           # Executando o comando wget.
	   if [[ $pod == vote-* ]];then
           kubectl exec -it $pod -n $namespace -- wget $IP_RESULT > /dev/null
           else
           kubectl exec -it $pod -n $namespace -- wget $IP_VOTE > /dev/null
           fi
           ...
           # concatenando os valores
           RESULT_TR_WGET=$(echo "$DIFFERENCE$MS" | bc)
	   ...
	   # Verifica se o pod usa imagem Alpine
            if [[ $(kubectl exec -it $pod -n $namespace -- cat /etc/os-release | grep -c "Alpine") -ne 0 ]]; then
                # testa o APk UPDATE usando o gerenciador de pacotes apk

                kubectl exec -it $pod -n $namespace -- apk update

                #RESULT_TR1_APT=$(kubectl exec -it $pod -n $namespace -- time apk update 2>&1 | grep -E 'real' | awk '{print $3}' | cut -d's' -f1 | awk '{printf "%.6f", $0}')
            else
                # Testa o APT usando o gerenciador de pacotes apt

                kubectl exec -it $pod -n $namespace -- apt update

                #RESULT_TR1_APT=$(kubectl exec -it $pod -n $namespace -- bash -c "time apt update" 2>&1 | grep -E 'real' | awk '{print $2}' | sed 's/[^0-9.]//g' | awk '{printf "%.6f", $0}')
            fi
	   ...
	   echo ""
           ...
           # concatenando os valores
           RESULT_TR_APT=$(echo "$DIFFERENCE$MS" | bc)

           #####################################  APT  ################################
           ####################  SAIDA DOS RESULTADOS INDIVIDUAL ######################

           echo "$(date); $Group; $pod; $RESULT_TR_PING; $RESULT_TR_WGET; $RESULT_TR_CURL; $RESULT_TR_APT" >> metricas_tempo_resposta_pod_$pod.csv

	   echo "$Group,$RESULT_TR_PING,$RESULT_TR_WGET,$RESULT_TR_CURL,$RESULT_TR_APT" >> metricas_para_R_pod_$pod.csv
           ...
           echo "$(date); $Group; $pod; $RESULT_TR_PING; $RESULT_TR_WGET; $RESULT_TR_CURL; $RESULT_TR_APT" >> metricas_tempo_resposta.csv
	   
	   echo "$(date +%d-%m-%Y),$Group,$pod,$RESULT_TR_PING,$RESULT_TR_WGET,$RESULT_TR_CURL,$RESULT_TR_APT" >> metricas_para_calculo.csv
